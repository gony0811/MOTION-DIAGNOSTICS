...
namespace EPLE.ImageProcessing
{
    public class HalconImageProcessing
    {
        ...
        public HalconImageProcessing(ILogger logger, DataManager dataManager)
        {
            this.logger = logger;
        ...
        public (double x, double y) Find(out bool find_result)
        {
            ...
            var diff_y = (ColumnCheck.D - cx) * resolution_x / 50.0 * -1;
            var diff_x = (RowCheck.D - cy) * resolution_y / 50.0 * -1;
            find_result = true;
        ...
        public BitmapImage ReadRegisteredModel()
        {
            ShapeModel.ReadShapeModel(modelFileName);
        ...
        public BitmapImage RegisterModel()
        {
            try
            {
                ...
                if (System.IO.File.Exists(registeredImageFileNameWithNoExtension))
                {
                    ...
                    {
                        // 파일이 다른 프로세스에서 사용 중인 경우, 파일 핸들을 해제하고 삭제를 시도합니다.
                        GC.Collect();
